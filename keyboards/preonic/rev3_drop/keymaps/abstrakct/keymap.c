...
const key_override_t altesc_key_override = ko_make_basic(MOD_MASK_ALT, KC_ESC, G(KC_L));
 ...
 * |------+------+------+------+------+------+------+------+------+------+------+------|
 * | Ctrl |NumPad| Alt  |Super |Lower |Space |Space |Raise | Lead | Four | Five |      |
 * `-----------------------------------------------------------------------------------'
 */
...
[_RAISE] = LAYOUT_preonic_grid(
  ...
  _______, KC_BTN1, KC_MS_U, KC_BTN2, KC_WH_U, _______, _______, RANGERCP,_______, _______, NO_PIPE, NO_QUOT,
  _______, KC_MS_L, KC_MS_D, KC_MS_R, KC_WH_D, _______, DOTDOT,  C(KC_B), _______, KC_SLSH, _______, NO_PLUS,
  _______, _______, _______, _______, CTSH_V,  _______, _______, CTSH_M,  _______, _______, ARROW,   NO_ACUT,
...
[_ADJUST] = LAYOUT_preonic_grid(
  KC_SYRQ, _______, _______, _______, _______,    _______, _______, _______,    _______,    _______, CTALT_0, _______,
  KC_CAPS, _______, DB_TOGG, RGB_TOG, RGB_M_P,    RGB_M_B, RGB_M_R, _______,    _______,    _______, _______, _______,
  _______, _______, MU_NEXT, AU_ON,   AU_OFF,     _______, _______, _______,    _______,    _______, _______, _______, // DF(_WORK),
  _______, AU_PREV, AU_NEXT, MU_ON,   MU_OFF,     MI_ON,   MI_OFF,  _______,    _______,    _______, _______, DF(_NORM),
  QK_BOOT, _______, _______, _______, TG(_LOWER), _______, _______, TG(_RAISE), TG(_MVMNT), _______, _______, TG(_GAME)
),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case FOUR:
            if (record->event.pressed) {
                layer_on(_FOUR);
                update_tri_layer(_FOUR, _FIVE, _ADJUST);
            } else {
                layer_off(_FOUR);
                update_tri_layer(_FOUR, _FIVE, _ADJUST);
            }
...
void matrix_scan_user(void) {
    ...
    //    // DCSS stuff
    //    // TODO: add a "dcss mode" or even separate layers
    //    SEQ_ONE_KEY(KC_D) {
    //        SEND_STRING("X>\n");
    //    }
    ...
    //    SEQ_ONE_KEY(KC_F) {
    //        SEND_STRING("xf" SS_DELAY(100) "\n");
    //    }

    //    SEQ_ONE_KEY(NO_OSTR) {
    //        register_code(NO_ARNG);
    //        unregister_code(NO_ARNG);
    //    }
    //    */
    //}
...
// Stuff that's kinda not actually used
